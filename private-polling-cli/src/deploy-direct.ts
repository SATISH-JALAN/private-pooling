...
async function main() {
  ...
  try {
    ...
    const unshieldedState = await waitForCondition(
      ...
      (s) => {
        const hasAny = Object.values(s.balances || {}).some((b) => (b as bigint) > 0n);
        return hasAny || !!(s as any).progress?.isStrictlyComplete?.();
      },
    ...
    for (let attempt = 1; attempt <= 5; attempt++) {
      try {
        ...
        const fresh = await withTimeout(
          ...
          25_000, 'fresh unshielded state',
        );

        const utxos = fresh.availableCoins.filter((c: any) => !c.meta.registeredForDustGeneration);
        ...
        const recipe = await withTimeout(
          walletProvider.wallet.registerNightUtxosForDustGeneration(
            utxos, ks.getPublicKey(),
            (p: Uint8Array) => ks.signData(p),
            (dustState as any).address,
          ),
          45_000, 'registerNightUtxosForDustGeneration()',
        );
        ...
        logger.warn(`[4/5] Attempt ${attempt}/5 failed: ${e instanceof Error ? e.message : e}`);
        if (attempt < 5) {
    ...
    const api = await withTimeout(
      PrivatePollingAPI.deploy(providers, logger),
      300_000, 'PrivatePollingAPI.deploy()',
    );
    ...
    if (walletProvider) try { await withTimeout(walletProvider.stop(), 10_000, 'stop'); } catch {}
    try { await withTimeout(testEnv.shutdown(), 10_000, 'shutdown'); } catch {}
